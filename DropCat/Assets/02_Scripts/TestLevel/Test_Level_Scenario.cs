...
public class Test_Level_Scenario : MonoBehaviour
{
    ...
    [SerializeField] private GameObject testTitleObj = null;
    private Coroutine levelCoroutine = null;
    ...
    private void LoadSequence_Intro()
    {
        Sequence SequenceLevel01 = new Sequence("Intro");
        Beat intro = new Beat();
        intro.AddEventsToBeatEnter(
            delegate
            {
                testTitleObj.SetActive(true);
                testTitleObj.GetComponent<TestLevelTitle>().TitleDisplay(2f);
            }
            );
        intro.AddEventsToBeatExit(delegate { 
            testTitleObj.SetActive(false); 
            chapterLevel_01.ContinueChapter();
        });
        SequenceLevel01.AddBeatToCurrent(intro);
        chapterLevel_01.AddSequenceToCurrent(SequenceLevel01);
    }

    private void LoadSequence_Test_Level_01()
    {
        Sequence SequenceLevel01 = new Sequence("Level_01");
